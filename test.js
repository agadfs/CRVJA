...
export const SeeTheMadeAlgorithmComponent = () => {
  ...
  const mapSize = 1000;
  const [width, setImageWidth] = useState(0);
  ...
  const processImage = () => {
    ...
    img.onload = () => {
      ...
      // Draw the original widthxwidth image onto the canvas (for pixel extraction)
      ...
      for (let y = 0; y < width; y++) {
        for (let x = 0; x < width; x++) {
          ...
          neighbors.forEach((neighbor) => {
            ...
            /* this removes all touches with equal rgb */
            /* if (currentRgb === neighborRgb) return; */
            /*  */

             /* this removes all touches with different rgb */
            /*  if (currentRgb !== neighborRgb) return;  */
            /*  */
      ...
      const sortedTouchCounter = Object.entries(touchCounter)
      .sort(([, countA], [, countB]) => countB - countA) // Sort descending by touch count
      .reduce((acc, [key, count]) => {
        acc[key] = count; // Rebuild the sorted object
        return acc;
      }, {});
        
      setRgbTouches(sortedTouchCounter);
  ...
  const generateRandomCanvas = () => {
    ...
    const pixelSize = 2; // Size of each square pixel in the random canvas
    ...
    <>
      ...
      {uploadedImage && (
        <div style={{maxHeight:"100px", overflow:"scroll", border:"1px solid black"}}>
          ...
          <canvas
            ref={canvasRef}
            width={width * multiplier} // width pixels * 10 = width0px width
            height={width * multiplier} // width pixels * 10 = width0px height
      ...
      {rgbTiles.length > 0 && (
        <div>
          ...
          <canvas
            ref={randomCanvasRef}
            width={mapSize * 2} // Set canvas size to 300x300
            height={mapSize * 2}
            style={{ border: "1px solid black" }}
          />
        </div>
